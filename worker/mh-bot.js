...
// ── Тексты follow-up ─────────────────────────────────────────────────────────
// A) Ушёл думать — мягкая забота + лёгкое касание боли + «мы на связи».
// ВАЖНО: бот НЕ знает контекста визита (программу не «составлял», боль не знает) —
// поэтому про причину прихода обобщённо, без выдуманных деталей и без давления.
function buildThinkFollowupText(name) {
  ...
  return `${hi}, что были сегодня у нас на консультации в Фабрике красивых тел M&M 🌸\n\n`
    + 'То, ради чего вы пришли, действительно важно — и это решаемо, шаг за шагом. '
    + 'Понимаем, что важное решение хочется обдумать, поэтому не торопим. Если останутся '
    + 'любые вопросы — по процедуре, результату или стоимости — мы всегда на связи, '
    + 'просто напишите 🤍';
}
